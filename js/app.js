...
function initAsync() {
     ...
     * @param   {function}  ack     Callback function responsible of returning
     *     acknowledgements
     */
...
function createGraph(graphContainer, toolbarContainer, outlineContainer) {
    ...
    addToolbarButton(graph, toolbarContainer, 'async', 'Async: Connecting', '',
        false);
    ...
    addToolbarButton(graph, toolbarContainer, 'actualSize', 'Actual Size', '',
        false);
                    ...
                    case 'progress_bar':
                        try {
                            ...
                            if (cellOrientation == 'vertical') {
                                cellGeometry.height = parseInt(
                                    value / progressBar.valueRatio);
                            }
                            else {
                                cellGeometry.width = parseInt(
                                    value / progressBar.valueRatio);
                            }
            ...
            case 'async':
                switch (asyncClient.getAsyncState()) {
                    ...
                    case 3:
                        ...
                        if (confirm(
                                'Do you want to reconnect into async server?')) {
                            asyncClient.reconnectSocket();
                            document.getElementById(
                                'toolbar-async').innerText = 'Async: Connecting';
                            document.getElementById(
                                'toolbar-async').style.color = 'blue';
                        }
                        break;
                    ...
                    case 1:
                        ...
                        if (confirm(
                                'Are you sure you want to disconnect async?')) {
                            asyncClient.close();
                        }
                        break;
                                    ...
                                    if (mxUtils.isNode(children[j].value)) {
                                        if (children[j].getAttribute('active') == 'yes' ||
                                            children[j].getAttribute('active')
                                                .toLowerCase() == 'true' ||
                                            parseInt(children[j].getAttribute('active')) == 1) {
                            ...
                            switch (cellEffectName) {
                                case 'flow':
                                    var state = graph.view.getState(
                                        allCells[i]);
                                    state.shape.node.getElementsByTagName(
                                        'path')[1].setAttribute('class',
                                        'flow-' + cellEffectValue);
                                    break;
                            }
                        });
                }
...
window.addEventListener('resize', function() {
    if (graph) {
        ...
        outlineDOM.style.width = containerDOM.getBoundingClientRect().width / 5;
        outlineDOM.style.height = containerDOM.getBoundingClientRect().height /
            5;
    }
});
